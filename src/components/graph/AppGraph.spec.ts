...
interface RendererMock {
	...
	graphData: ReturnType<
		typeof vi.fn<(graphData: RendererGraphPayload) => RendererMock>
	>;
...
const createProps = (
	overrides: Partial<AppGraphProps> = {},
): AppGraphProps => ({
	graph,
...
test("AppGraph passes graph data to force renderer", async () => {
	...
	expect(graphPayload.links).toEqual([
...
test("AppGraph keeps canvas hidden until initial fit completes", async () => {
	...
	await flushGraphRender();
	expect(wrapper.get('[data-test="graph-canvas"]').classes()).not.toContain(
		"ready",
	);

	vi.advanceTimersByTime(2000);
	await flushGraphRender();
	expect(wrapper.get('[data-test="graph-canvas"]').classes()).toContain(
		"ready",
	);
	vi.useRealTimers();
});
