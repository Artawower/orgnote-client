import type { Extension, ExtensionManifest, OrgNoteApi } from 'orgnote-api';
...
export const orgInlineMarkupExtension: Extension = {
  onMounted: async (api: OrgNoteApi): Promise<void> => {
    console.log('[line 18]: ORG INLINE MARKUP');
    const editorStore = api.core.useEditor();
    const { createWidgetBuilder, addWidgets } = editorStore;
