...
test('registerFileSystem should add new filesystem', async () => {
  const store = useFileSystemStore();
  await store.registerFileSystem('test-fs', mockFileSystem);

  expect(store.fileSystems['test-fs']).toStrictEqual(mockFileSystem);
});

test('pickFileSystem should set active filesystem', async () => {
  const store = useFileSystemStore();
  await store.registerFileSystem('test-fs', mockFileSystem);
  await store.pickFileSystem('test-fs');

  expect(store.activeFileSystemName).toBe('test-fs');
});

test('unregisterFileSystem should remove filesystem', async () => {
  const store = useFileSystemStore();
  await store.registerFileSystem('test-fs', mockFileSystem);
  await store.unregisterFileSystem('test-fs');

  expect(store.fileSystems['test-fs']).toBeUndefined();
});
